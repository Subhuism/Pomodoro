...
const TaskPanel = ({ themeColor }) => {
    ...
    return (
        <div className="brutal-card p-6 h-full flex flex-col hover:-translate-x-1 hover:-translate-y-1 hover:shadow-none transition-all" style={{
                ...
                <div className="space-y-3 ">
                    ...
                    {tasks.map((task) => (
                        <div
                            ...
                            <button onClick={() => deleteTask(task.id)} className="px-2 font-bold justify-between items-center ml-auto hover:cursor-pointer"><Trash2 /></button>
                        </div>))}
                </div>
            </div>
            <div className="">
                ...
                <button
                    className=" w-full border-4 border-black bg-yellow-400 py-3 font-bold max-h-90 overflow-y-auto hover:border-[#060dd4] hover:text-[#060dd4] hover:cursor-pointer transition-all duration-700" onClick={addTask} >+ ADD TASK </button>
            </div>
        </div>
    )
}
